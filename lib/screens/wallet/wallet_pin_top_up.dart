...
class WalletPinTopUp extends StatefulWidget {
  WalletPinTopUp({
    super.key,
  });

  @override
  State<WalletPinTopUp> createState() => _WalletPinTopUpState();
}

class _WalletPinTopUpState extends State<WalletPinTopUp> {
  // Property? _property;
  bool loading = false;
  // int _quantity = 0;
  bool _success = false;
  ...
  void _initPinManagement() {
    try {
      ...
      CardModel? card =
          Provider.of<PaymentProvider>(context, listen: false).selectedCard;
      setState(() {
        _topuppayment = topuppayment;
        _card = card;
      });
    } catch (e) {
      showErrorDialog("could not init pin", context);
    }
  }
                            ...
                            onPressed: () async {
                              ...
                              try {
                                ...
                                await Provider.of<PaymentProvider>(context,
                                        listen: false)
                                    .chargeAlreadyKnownCard(
                                        _card,
                                        double.parse("$_topuppayment").ceil(),
                                        _pinController.text);
