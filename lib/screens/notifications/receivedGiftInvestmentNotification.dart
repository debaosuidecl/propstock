...
class ReceivedGiftInvestmentNotification extends StatelessWidget {
  final NotificationModel notif;
  final void Function(String?)? execute;
  const ReceivedGiftInvestmentNotification(
          ...
          Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Text(
                    "${receivedFrom == null ? "User" : receivedFrom.firstName}",
                    style: TextStyle(
                      fontSize: 16,
              ...
              Container(
                ...
                child: RichText(
                  text: TextSpan(
                    text:
                        'You just received an investment gift. Deadline to accept is ',
                    style: TextStyle(
                      fontSize: 14,
                      ...
                      onTap: () async {
                        try {
                          await Provider.of<NotificationProvider>(context,
                                  listen: false)
                              .acceptOrRejectGift(
                                  "accept", receivedFrom!.id, notif.assetid);

                          execute!(
                              "${notif.createdAt}xxxxxxInvestment Gift Accepted");
                        } catch (e) {
                      ...
                      onTap: () async {
                        try {
                          await Provider.of<NotificationProvider>(context,
                                  listen: false)
                              .acceptOrRejectGift(
                                  "reject", receivedFrom!.id, notif.assetid);

                          execute!(
                              "${notif.createdAt}xxxxxxInvestment Gift Rejected");
                        } catch (e) {
                          showErrorDialog(e.toString(), context);
                        }
                      },
